...
【成绩口径·重要】本次填写的「各科成绩」是【中考成绩】，请一律按「中考各科得分率」来分析，而不是把中考绝对分当高中分：
- 用各科得分率判断该生学科基础强弱（如数学 95% 说明初中底子厚、物理 72% 提示高中易分化）；
- 基于得分率预测进入高中后的学科适配度与潜在断层（哪些初中能力可迁移、哪些断层会在高中被放大）；
- 禁止把中考分数直接说成"当前高中水平"，要写明"这是中考口径，高中首次大考后会校准"。
...
【输出结构（5 模块，全文 1200–1800 字，规划模块必须结构化分层）】
1. 画像解读（≤200 字）：3–5 个精准标签；先扬后抑，必须锚定具体分数。
2. 各科定位（主体）：一张表（科目 / 当前分 / 位次落差 / 能力要求变化）+ 每科一句点评。
3. 学年规划（主体，必须分层分区、可扫读，禁止纯大段散文）：
   - 先给「总览」一句话（≤60 字）定调；
   - 再用一张「阶段表」分行呈现，列 = 阶段 / 时间锚点 / 重点任务 / 里程碑，按该年级时间锚点逐行，每行信息密度高、不啰嗦；
   - 最后给「关键动作」3–5 条，每条必须明确【谁执行 / 在哪执行 / 多久检查一次】，且与休息安排硬约束匹配；
   - 高一在阶段表后附「适应期观察清单」（≤5 条，每条≤30 字，句式"看什么 → 出现什么信号就动手"），高二高三不出现；
   - 规模控制：总览≤60 字 + 阶段表 4–6 行 + 关键动作≤5 条，整体清爽。
...
【上传材料处理（若用户提供）】
- 若用户提供了【上传材料】，报告必须紧密结合材料中的事实依据：学生的当前学习状态、家长填写的诉求与沟通风格、已暴露的问题，都应在「画像解读 / 各科定位 / 学年规划 / 致家长的话」中具体体现，禁止泛泛而谈或忽略材料。
- 材料优先于空泛推断；材料中的客观事实与成绩解读冲突时，以材料事实为准，并在相应处点出冲突。
- 结合材料能让报告更贴切家长/学生真实情况，这是本报告的加分项而非可选项。

【合规红线】
- 允许出现：有道领世老师姓名、教龄/职称、教学主张。
- 禁止出现：除有道领世外的任何机构/平台/老师；手机号、微信号、班级群等联系方式；学员全名或"学校+姓名"可定位组合。
（参考案例库暂关闭，重建后在此补回「自有真实提分案例 / 案例匹配」指令）

仅输出一个 JSON 对象（紧凑、无缩进、无 markdown 包裹）：
{"portrait":"画像解读文本(≤200字)","subjects":[{"subject":"科目","score":数字或null,"gap":"位次落差描述","requirement":"能力要求变化","comment":"一句点评"}],"plan":{"overview":"学年规划一句话总览(≤60字)","stages":[{"phase":"阶段名(如 适应期)","anchor":"时间锚点(如 9-10月)","focus":"本阶段重点任务","output":"阶段里程碑/产出"}],"actions":["关键动作(明确谁执行/在哪执行/多久检查一次)","..."]},"adapt_checklist":["清单条1",...],"teacher_advice":["建议1","建议2","建议3"],"target_outlook":"目标院校前瞻(仅填目标时出现)","parent_words":"致家长的话(≤250字)"}
注意：plan 必须是对象（含 overview/stages/actions 三个键），禁止把整段规划塞进一个字符串；stages 为 4–6 行、actions 为 3–5 条。`;
...
【成绩来源与口径（关键判断条件，必须执行）】
${ctx.grade === "高一"
  ? "本次「各科成绩」=【中考成绩】。分析必须以「中考得分率」为核心：用各科得分率预测高中学科适配度与潜在分化，明确「哪些初中能力可迁移、哪些断层会在高中放大」，禁止把中考绝对分当作高中在校分，必须注明「这是中考口径，高中首次大考后校准」。"
  : "本次「各科成绩」=【高中在校实际成绩】。直接据此分析当前学科真实状态、位次落差与提分空间，不要回退到中考口径、不要再用得分率预测框架。"}

【上传材料】${ctx.material ? "" : "（无）"}
${ctx.material || ""}${ctx.material ? "\n⚠️ 必须结合以上【上传材料】分析：材料中学生的具体情况、家长填写的诉求与沟通风格，都应在报告中具体体现，严禁忽略或仅泛泛提及。" : ""}
...
// 功能开关：参考案例库暂关闭（用户将另行整理、统一建库）。改为 true 并补回 prompts 中的案例指令即可恢复。
const CASE_ENABLED = false;
// 返回与某科/某分数最贴近的 1–2 条匿名案例（|起分-分数|≤10，同省优先）；关闭时恒返回空
function matchCases(subject, score, province) {
  const lib = (typeof window !== "undefined" && window.CASE_LIB) || [];
  if (!CASE_ENABLED || !lib.length || score == null) return [];
